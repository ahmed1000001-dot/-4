...
public class MainActivity extends Activity{
 ...
 public class Bridge{
  ...
  @JavascriptInterface public long addPurchaseInvoice(String j){return db.addPurchaseInvoice(j);}@JavascriptInterface public int updatePurchaseInvoice(long id,String j){return db.updatePurchaseInvoice(id,j);}@JavascriptInterface public int deletePurchaseInvoice(long id){return db.deletePurchaseInvoice(id);}
  ...
  @JavascriptInterface public long addSupplierReturn(String payload){return db.addSupplierReturn(payload);}@JavascriptInterface public int updateSupplierReturn(long id,String payload){return db.updateSupplierReturn(id,payload);}@JavascriptInterface public int deleteSupplierReturn(long id){return db.deleteSupplierReturn(id);}
  ...
  @JavascriptInterface public void exportBackup(){pending=db.backup();runOnUiThread(()->{Intent i=new Intent(Intent.ACTION_CREATE_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("application/json");i.putExtra(Intent.EXTRA_TITLE,"canteen-pro-v5.2-backup.json");startActivityForResult(i,EXP);});}
